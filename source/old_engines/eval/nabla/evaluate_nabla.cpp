...
namespace Eval
{
...
#if 0
		// KPP_KKPT型からfe_endの拡張をしながら読み込むときは、こちら。
		//		auto input = EvalIO::EvalInfo::build_kpp_kkpt32(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN));
#else
		auto input = EvalIO::EvalInfo::build_nabla(make_name(KK_BIN), make_name(KKP_BIN), make_name(KPP_BIN));
#endif
		auto output = EvalIO::EvalInfo::build_nabla((void*)kk, (void*)kkp, (void*)kpp);

		// cf. expand_kpp_to_nabla1()

		// 評価関数の実験のためにfe_endをbuild_kpp_kkpt()の値から変更しているかも知れないので現在のfe_endの値をもとに読み込む。
	...
	// nabla用のmir/invテーブルの初期化。
	void init_nabla_mir_inv_tables();

	void expand_kpp_to_nabla1();
	...
	void init()
	{
		...
		// nabla用のmir/invテーブルの初期化。
		mir_piece_init_function = [] { init_nabla_mir_inv_tables(); };

		// evaluate()のなかでどうしてもinv_piece()を使いたいのでここで初期化する。
		init_mir_inv_tables();

		//expand_kpp_to_nabla1();
	}
	...
	void load_eval()
	{
		...
		auto mapped_file_name = TEXT("YANEURAOU_NABLA_MMF" ENGINE_VERSION) + cv.from_bytes(dir_name);
		auto mutex_name = TEXT("YANEURAOU_NABLA_MUTEX" ENGINE_VERSION) + cv.from_bytes(dir_name);
	...
	// BonaPieceの文字列化(nabla対応)
	auto bp_str = [](BonaPiece p)
	{
		std::stringstream ss;
		if (p >= fe_old_end)
		{
	...
	void print_eval_list(Position& pos)
	{
		...
		for (int i = 0; i < length; ++i)
			cout << int(list_fb[i]) << " = " << bp_str(list_fb[i]) << endl;
	...
	void print_eval_stat(Position& pos)
	{
		...
		// 38枚の駒を表示
		for (i = 0; i < length ; ++i)
			cout << int(list_fb[i]) << " = " << bp_str(list_fb[i]) << " , " << int(list_fw[i]) << " =  " << bp_str(list_fw[i]) << endl;
		...
		for (i = 0; i < length ; ++i)
		{
			...
			for (j = 0; j < i; ++j)
			{
				...
				cout << "BKPP : " << sq_bk << " " << k0 << " " << bp_str(l0) << " = " << kpp_kp_fb[l0] << endl;
				cout << "WKPP : " << sq_wk << " " << k1 << " " << bp_str(l1) << " = " << kpp_kp_fw[l1] << endl;
			...
			cout << "KKP : " << sq_bk << " " << sq_wk << " " << bp_str(k0) << " = " << kkp[sq_bk][sq_wk][k0][0] << " + " << kkp[sq_bk][sq_wk][k0][1] << endl;
	...
	void DirtyPiece::add_piece(Square sq, Piece pc)
	{
		...
		// nabla対象の歩の場合だけ特殊な処理が必要。
		if (type_of(pc) == PAWN)
		{
			// state()->work[0]が先手1-5筋の歩、work[1]が先手9-5筋の歩、work[2]が後手9-5筋の歩、work[3]が後手1-5筋の歩の
			// BonaPiece値になっているものとする。

			Color c = color_of(pc);
			if (c == BLACK)
			{
				Rank r = rank_of(sq);
				if (RANK_5 <= r && r <= RANK_7) // この3段にいるならば
				{
					int r2 = (6 - (int)r) + 1; // 7段目が1、6段目が2、5段目が3
					File f = file_of(sq);

					// f == FILE_5だと、以下の両方のifが成立して2回add_listされることに注意。

					if (f <= FILE_5)
					{
						// 4進数のf桁目をr2が表現していると考える。
						auto& v = state()->nabla_work[0];
						if (v)
							remove_list.push_back((BonaPiece)(v + fe_nabla_pawn));
						v += r2 << ((int)f * 2);
						add_list.push_back((BonaPiece)(v + fe_nabla_pawn));
					}
					if (f >= FILE_5)
					{
						auto& v = state()->nabla_work[1];
						if (v)
							remove_list.push_back((BonaPiece)(v + 1024 + fe_nabla_pawn));
						v += r2 << ((8-(int)f) * 2);
						add_list.push_back((BonaPiece)(v + 1024 + fe_nabla_pawn));
					}
				}
				else
					goto NORMAL; // 対象外。通常処理
			}
			else {
				// WHITE

				Rank r = rank_of(sq);
				if (RANK_3 <= r && r <= RANK_5) // この3段にいるならば
				{
					int r2 = ((int)r) - 1; // 3段目が1、4段目が2、5段目が3
					File f = file_of(sq);

					if (f >= FILE_5)
					{
						// 4進数のf桁目をr2が表現していると考える。
						auto& v = state()->nabla_work[2];
						if (v)
							remove_list.push_back((BonaPiece)(v + 1024*2 + fe_nabla_pawn));
						v += r2 << ((8 - (int)f) * 2);
						add_list.push_back((BonaPiece)(v + 1024*2 + fe_nabla_pawn));
					}
					if (f <= FILE_5)
					{
						auto& v = state()->nabla_work[3];
						if (v)
							remove_list.push_back((BonaPiece)(v + 1024*3 + fe_nabla_pawn));
						v += r2 << ((int)f * 2);
						add_list.push_back((BonaPiece)(v + 1024*3 + fe_nabla_pawn));
					}
				}
				else
					goto NORMAL; // 対象外。通常処理
			...
			if (c == BLACK)
			{
				...
				if (RANK_5 <= r && r <= RANK_7) // この3段にいるならば
				{
					...
					if (f <= FILE_5)
					{
						...
						auto& v = state()->nabla_work[0];
						remove_list.push_back((BonaPiece)(v + fe_nabla_pawn));
						ASSERT_LV3(v >= (r2 << ((int)f * 2)));
						v -= r2 << ((int)f * 2); // ここに駒があるはずなのだが。
						if (v)
							add_list.push_back((BonaPiece)(v + fe_nabla_pawn));
					}
					if (f >= FILE_5)
	...
	void DirtyPiece::remove_and_add_piece(Square from, Piece moved_pc, Square to, Piece moved_after_pc)
	{
		...
		if (type_of(moved_pc) != PAWN)
		{
			...
			Color c = color_of(moved_pc);
			Rank r = rank_of(to);
			if (c == BLACK)
			{
				if (RANK_5 <= r && r <= RANK_6)
				{
					int r2 = (6 - (int)r) + 1; // 7段目が1、6段目が2、5段目が3
					File f = file_of(to);
					...
					{
						auto& v = state()->nabla_work[1];
						remove_list.push_back((BonaPiece)(v + 1024*1 + fe_nabla_pawn));
						v -= (r2 - 1) << ((8 - (int)f) * 2);
						v += (r2    ) << ((8 - (int)f) * 2);
						add_list.push_back((BonaPiece)(v + 1024*1 + fe_nabla_pawn));
					}
				}
				else
					goto NORMAL;
	...
	NORMAL:
		remove_piece(from, moved_pc);
		add_piece(to, moved_after_pc);

	}

	// 以下は手駒の処理。手駒は関係ないので元あったdefault実装のままで良い。
	...
	void init_nabla_mir_inv_tables()
	{
		for (BonaPiece p = fe_nabla_pawn; p < fe_nabla_pawn + 4096; ++p)
		{
			int type = (p - fe_nabla_pawn) / 1024;

			// 設計が良いのでmirはこれだけで達成できる
			mir_piece_[p] = p + ((type == 0 || type == 2) ? 1024 : -1024);

			// 設計が良いのでinvはこれだけで達成できる
			inv_piece_[p] = p + ((type < 2) ? 1024*2 : -1024*2);
		}

	}

	// nablaのpiece_listのvalidation
	void is_valid_nabra_eval_list(const Position& pos)
	{
		auto& eval_list = *pos.eval_list();
		auto& work = pos.state()->nabla_work;

		for (int i = 0; i < 4; ++i)
		{
			std::vector<BPInfo> a;
			map_from_nabla_pawn_to_bp((BonaPiece)(work[i] + fe_nabla_pawn + 1024 * i) , a);

			for (auto p : a)
			{
				auto sq = p.f | p.r;
				if (pos.piece_on(sq) != p.pc)
				{
					cout << pos << endl;
					for (auto p : a)
					{
