...
/*
	...
	そもそも、Node,Child構造体は無限に増えていくので、そんなところに同期化のためのデータを保持しているのが設計上の誤り。
	...
	public:
		...
		Move mate_dfpn(Position& pos, u32 nodes_limit)
		{
			...
			if (current_root->pn == 0 && current_root->dn >= NodeType::DNPN_MATE)
			{
				...
				return pick_the_best<true, true>(node);
			}
		...
		// ※　最短手順である保証はない。
		std::vector<Move> get_pv() const { return get_pv<true>(); }

		// 不詰の証明をするPVの取得。
		// mate_dfpn()がMOVE_NULL(不詰)を返した時に、そのPVを取得する。
		// (不詰の手順。攻め方はなるべく長め(最長ではない)、受け方はなるべく短め(最短ではない)に逃れる)
		std::vector<Move> get_unproof_pv() const { return get_pv<false>(); }
		...
		bool is_out_of_memory() const { return node_manager.is_out_of_memory(); }
	...
	protected:
		...
		std::vector<Move> get_pv() const {
			...
			while (node) {
				...
				Move move = or_node ? pick_the_best<true,proof>(node) : pick_the_best<false,proof>(node);
				if (move == MOVE_NONE)
					break;
		...
		template <bool or_node, bool proof>
		Move pick_the_best(NodeType*& node) const
		{
			NodeType* children = node->children;
		...
		void SummarizeNode(NodeType* node)
		{
			...
			NodeType* children = node->children; // nullptrでありうるが、そのときはchild_num == 0;
		...
		void ExpandNode(Position& pos, NodeType* node)
		{
			...
			if (child_num == 0)
			{
				...
				// 忘れないうちにぶら下げておく。
				node->set_child(child_num , children );
