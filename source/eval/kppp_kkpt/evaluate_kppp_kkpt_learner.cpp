...
#if defined(EVAL_LEARN) && defined(EVAL_KPPP_KKPT)
...
namespace Eval
{
	...
	void add_grad(Position& pos, Color rootColor, double delta_grad , const std::array<bool, 4>& freeze)
	{
		...
		memcpy(list_fb, pos_.eval_list()->piece_list_fb(), sizeof(BonaPiece) * (size_t)PIECE_NUMBER_KING);
		memcpy(list_fw, pos_.eval_list()->piece_list_fw(), sizeof(BonaPiece) * (size_t)PIECE_NUMBER_KING);
		...
		for (i = 0; i < PIECE_NUMBER_KING; ++i)
		{
			...
			for (j = 0; j < i; ++j)
			{
				...
				if (!freeze_kpp)
				{
					...
					weights_kpp[g_kpp.fromKPP(inv_sq_wk, k1, l1).toRawIndex()].add_grad(g_flip[0]);
				}
...
#pragma omp for schedule(dynamic,20000)
			for (s64 index_ = 0; index_ < (s64)vector_length; ++index_)
			{
				...
				else if (g_kppp.is_ok(index) && !freeze_kppp)
				{
					...
					if (weights_kppp[raw_index].get_grad() != 0)
					{
						auto& v = kppp_[raw_index];
						weights_kppp[raw_index].updateFV(v);
						weights_kppp[raw_index].set_grad(zero);
					}
#endif
				}
			}
