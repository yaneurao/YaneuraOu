...
namespace Eval
{
	...
	void init_grad(double eta1, u64 eta1_epoch, double eta2, u64 eta2_epoch, double eta3)
	{
		...
		ASSERT(KPPP::triangle_fe_end == Eval::kppp_triangle_fe_end);
		KPPP::king_sq = Eval::KPPP_KING_SQ;
		...
		// KPPP
		u64 size_kppp = KPPP::max_index() - KPPP::min_index();
	...
	void add_grad(Position& pos, Color rootColor, double delta_grad , const std::array<bool, 4>& freeze)
	{
		...
		if (kppp_bk_enable)
		{
#if defined(USE_KPPP_KKPT_MIRROR)
		...
		for (i = 0; i < PIECE_NO_KING; ++i)
		{
			...
			for (j = 0; j < i; ++j)
			{
				...
				if (!freeze_kpp)
				{
					weights_kpp[KPP(sq_bk    , k0, l0).toIndex() - KPP::min_index()].add_grad(g);
					weights_kpp[KPP(inv_sq_wk, k1, l1).toIndex() - KPP::min_index()].add_grad(g_flip);
				}
				...
				case 1:
					...
					{
						m0 = list_fb[k];
						weights_kppp[KPPP(sq_bk_for_kppp, m0, l0, k0).toIndex() - KPPP::min_index()].add_grad(g);
					}
					break;
				case 2:
					// WKPPP
					...
					{
						m1 = list_fw[k];
						weights_kppp[KPPP(sq_wk_for_kppp, m1, l1, k1).toIndex() - KPPP::min_index()].add_grad(g_flip);
					}
					break;
				case 3:
					// KPPP
	...
	{
		u64 vector_length = KPPP::max_index();
...
#pragma omp for schedule(dynamic,20000)
			for (s64 index_ = 0; index_ < (s64)vector_length; ++index_)
			{
				...
				if (index < KPPP::min_index() && !min_index_flag[index])
					continue;
				...
				else if (KPP::is_ok(index) && !freeze_kpp)
				{
					...
					for (auto id : ids)
						weights_kpp[id - KPP::min_index()].set_grad(zero_t);
				}
				else if (KPPP::is_ok(index) && !freeze_kppp)
				{
					...
					std::array<LearnFloatType, 2> g_sum = zero_t;
					for (auto id : ids)
						g_sum += weights_kppp[id - KPPP::min_index()].get_grad();

					if (is_zero(g_sum))
						continue;

					auto& v = kppp_ksq_pcpcpc(a[0].king(),a[0].piece0(),a[0].piece1(),a[0].piece2());
					weights_kppp[ids[0] - KPPP::min_index()].set_grad(g_sum);
					weights_kppp[ids[0] - KPPP::min_index()].updateFV(v);

					for (int i = 1; i < KPPP_LOWER_COUNT; ++i)
						kppp_ksq_pcpcpc(a[i].king(),a[i].piece0(),a[i].piece1(),a[i].piece2()) = v;

					for (auto id : ids)
						weights_kppp[id - KPPP::min_index()].set_grad(zero_t);
#else

					// 次元下げなしとわかっていれば、もっと単純化された速いコードが書ける。
					u64 index2 = index - KPPP::min_index();
					if (!is_zero(weights_kppp[index2].get_grad()))
					{
						auto& v = ((ValueKppp*)kppp)[index2];
						weights_kppp[index2].updateFV(v);
						weights_kppp[index2].set_grad(zero_t);
					}
#endif
				}
			}
