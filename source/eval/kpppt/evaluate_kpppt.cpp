...
namespace Eval
{
	...
	ValueKppp (*kppp_)[1] /*[KPPP_KING_SQ][kppp_triangle_fe_end]*/;
	...
	void eval_assign(void* ptr)
	{
		...
		kppp_ = (ValueKppp(*)[1])                                  (p + size_of_kk + size_of_kkp + size_of_kpp);
	}
	...
	void load_eval()
	{
		...
		auto mapped_file_name = TEXT("YANEURAOU_KPPP_KKPT_MMF" ENGINE_VERSION) + cv.from_bytes(dir_name);
		auto mutex_name = TEXT("YANEURAOU_KPPP_KKPT_MUTEX" ENGINE_VERSION) + cv.from_bytes(dir_name);
			...
			for (i = 0; i < PIECE_NO_KING; ++i)
			{
				...
				for (j = 0; j < i; ++j)
				{
					...
					for (k = 0; k < j; ++k)
					{
						...
						ASSERT_LV3(m0 > l0 && l0 > k0);
						sum_bkppp += kppp_ksq_pcpcpc(sq_bk_for_kppp, m0 , l0 , k0);
					}
				}
				// KKP
				sum.p[2] += kkp[sq_bk][sq_wk][k0];
			...
			for (i = 0; i < PIECE_NO_KING; ++i)
			{
				...
				for (j = 0; j < i; ++j)
				{
					...
					for (k = 0; k < j; ++k)
					{
						...
						ASSERT_LV3(m1 > l1 && l1 > k1);
						sum_wkppp += kppp_ksq_pcpcpc(sq_wk_for_kppp, m1 , l1 , k1);
					}
				}
				// KKP
				sum.p[2] += kkp[sq_bk][sq_wk][k0];
