    ...
    for (auto king_sq : SQ)
        for (auto sq : SQ)
            for (int m = 0; m < 11; ++m)  // 利きの数
            {
                ...
                our_effect_table[king_sq][sq][m] =
                  multi_effect_value[m] * our_effect_value[d] / (1024 * 1024);
                their_effect_table[king_sq][sq][m] =
                  multi_effect_value[m] * their_effect_value[d] / (1024 * 1024);
            }
...
Value compute_eval(const Position& pos) {
    ...
    for (auto sq : SQ)
    {
        // 盤上の升の利きの価値
        score +=
          effect_table[pos.king_square(BLACK)][pos.king_square(WHITE)][sq]
                      [pos.board_effect[BLACK].effect(sq)][pos.board_effect[WHITE].effect(sq)];
    ...
    for (auto king_sq : SQ)
        for (auto sq : SQ)
            for (int m = 0; m < 3; ++m)  // 利きの数
            {
                ...
                our_effect_table[king_sq][sq][m] =
                  double(multi_effect_value[m] * our_effect_value[d]) / (1024 * 1024);
                their_effect_table[king_sq][sq][m] =
                  double(multi_effect_value[m] * their_effect_value[d]) / (1024 * 1024);
            }
...
Value compute_eval(const Position& pos) {
    ...
    for (auto sq : SQ)
        score += KKPEE[pos.king_square(BLACK)][pos.king_square(WHITE)][sq]
                      [std::min(int(pos.board_effect[BLACK].effect(sq)), 2)]
                      [std::min(int(pos.board_effect[WHITE].effect(sq)), 2)][pos.piece_on(sq)];
                ...
                for (auto sq : SQ)
                    for (int m1 = 0; m1 < 3; ++m1)      // 先手の利きの数
                        for (int m2 = 0; m2 < 3; ++m2)  // 後手の利きの数
                        {
                            ...
                            // 先手から見たスコア
                            KKPEE[king_black][king_white][sq][m1][m2][pc] =
                              int16_t(score * FV_SCALE);
                        }
}

// KKPEE9 評価関数本体(わずか8行)
// 変数名短くするなどすれば１ツイート(140文字)に収まる。
Value comcompute_evalpute_eval(const Position& pos) {
...
void init() {
    ...
    int king_pos_bonus[] = {875, 655, 830, 680, 770, 815, 720, 945, 755, 605, 455, 610, 595, 730,
                            610, 600, 590, 615, 565, 640, 555, 525, 635, 565, 440, 600, 575, 520,
                            515, 580, 420, 640, 535, 565, 500, 510, 220, 355, 240, 375, 340, 335,
                            305, 275, 320, 500, 530, 560, 445, 510, 395, 455, 490, 410, 345, 275,
                            250, 355, 295, 280, 420, 235, 135, 335, 370, 385, 255, 295, 200, 265,
                            305, 305, 255, 225, 245, 295, 200, 320, 275, 70,  200};
                    ...
                    for (int m1 = 0; m1 < 3; ++m1)      // 先手の利きの数
                        for (int m2 = 0; m2 < 3; ++m2)  // 後手の利きの数
                        {
                            ...
                            for (auto color : COLOR)  // 両方の玉に対して
                            {
                                ...
                                if (dist(sq, king_sq) == 1)
                                {
                                    ...
                                    if (effect_us <= 1)
                                        ...
                                        score -=
                                          double((pc == NO_PIECE || color_of(pc) != color) ? 11
                                                                                           : -20)
                                          * (color == BLACK ? 1 : -1);
                                    else
                                        // 自玉以外の利きがある。この時、そこが空きか敵駒なら、減点なし。さもなくば9点加点。
                                        score -=
                                          double((pc == NO_PIECE || color_of(pc) != color) ? 0
                                                                                           : -11)
                                          * (color == BLACK ? 1 : -1);
                                }
                            }
                            ...
                            else if (type_of(pc) == KING)
                            {
                                ...
                                score +=
                                  color_of(pc) == BLACK
                                    ? king_pos_bonus[(FILE_9 - file_of(sq)) + int(rank_of(sq) * 9)]
                                    : -king_pos_bonus[(FILE_9 - file_of(Inv(sq)))
                                                      + int(rank_of(Inv(sq)) * 9)];
                            }
                            else
                            {
                ...
                for (auto sq : SQ)
                    for (int m1 = 0; m1 < 3; ++m1)      // 先手の利きの数
                        for (int m2 = 0; m2 < 3; ++m2)  // 後手の利きの数
                        {
                            ...
                            score += our_effect_table[king_black][sq][m1]
                                   * our_effect_rate[direction_of(king_black, sq)] / 1024;
                            score -= their_effect_table[king_black][sq][m2]
                                   * their_effect_rate[direction_of(king_black, sq)] / 1024;
                            score -= our_effect_table[Inv(king_white)][Inv(sq)][m2]
                                   * our_effect_rate[direction_of(Inv(king_white), Inv(sq))] / 1024;
                            score += their_effect_table[Inv(king_white)][Inv(sq)][m1]
                                   * their_effect_rate[direction_of(Inv(king_white), Inv(sq))]
                                   / 1024;
...
// MaterialEvaluatorの実装。
void  MaterialEvaluator::init() { init(); }
Value MaterialEvaluator::evaluate(const Position& pos) { return compute_eval(pos); }
